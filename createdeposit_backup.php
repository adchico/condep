...
// validate input
    ...
    //    $GlobalBLNo = $_POST['textBL'];
        ...
        $sql = "INSERT INTO tbl_deposit(DepBLNo,DateOfDeposit,ContainerNo, housebl, ReceiptNo,StubNoRefNo,DepAmount,CtrlFormNo,CheqNo,deduction_desc, cheq_deduction,DepStatus,id_BLNo,Encodedby,EncodedDateDeposit) values(?, ?, ?, ?,?,?,?,?, ?, ?, ?, ?, ?, ?, ?)";
        $q = $pdo->prepare($sql);
        $q->execute(array($DepBLNoCD,$DateOfDeposit,$ContainerNo,$housebl,$ReceiptNo,$StubNoRefNo,$DepAmount,$CtrlFormNo,$CheqNo,$deduction_desc,$cheq_deduction,$DepStatus,$id_BLNo,$Encodedby,$EncodedDateDeposit));
...
<?php include "blinfo.php"; ?>
         ...
         <form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" >
                  ...
                  <div class="control-group <?php// echo !empty($DepBLNoError)?'error':'';?>">
                  ...
                  <div class="control-group <?php echo !empty($houseblError)?'error':'';?>">
                      ...
                      <div class="controls">
                          ...
                          <div class="control-group <?php echo !empty($ReceiptNoError)?'error':'';?>">
                            ...
                            <div class="controls">
                                <input id="upCase"  name="ReceiptNo" type="text"  placeholder="ReceiptNo" <?php if($row['ShippingLine'] == 'APL'){echo 'value=APC-MNL-';} elseif($row['ShippingLine'] == 'YANG MING'){echo 'value=MNL/CDEP/';} elseif($row['ShippingLine'] == 'COSCO'){echo 'value=CDR';} elseif($row['ShippingLine'] == 'HYUNDAI'){echo 'value=HPH';} else {echo 'value=""';}?>
                          ...
                          <div class="control-group <?php echo !empty($CtrlFormNoError)?'error':'';?>">
                            ...
                            <div class="controls">
                                ...
                                <input id="upCase" style="background-color: #99b9ff;"  maxlength="22" name="CtrlFormNo" type="text"  placeholder="NA for No CtrlFormNo" value="<?php echo !empty($CtrlFormNo)?$CtrlFormNo:'';?>">
                                  <?php if (!empty($CtrlFormNoError)): ?>
                                      <span class="help-inline"><?php echo $CtrlFormNoError;?></span>
                                  <?php endif;?> (Consignee)-SLYRMONTHDAY-(TIN/DON)-NO
                          ...
                          <div class="control-group <?php echo !empty($cheq_deductionError)?'error':'';?>">
                                                      ...
                                                      <div class="controls">
                            <input class="number" id="inputbackcolor" maxlength="25" name="cheq_deduction" type="number"  placeholder="leave empty if no deduction" value="<?php echo !empty($cheq_deduction)?$cheq_deduction:'';
                            ?>">
...
<!- DepStatus Select ->
                          ...
                          $pdo = Database::connect();
                          $smt_depstatus = $pdo->prepare('SELECT depstatus From tbl_depstatus');
