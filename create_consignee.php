...
include_once 'logstat.php';
    ...
    if ( !empty($_POST)) {
        ...
        if (empty($Authorize1)) {
            $Authorize1Error = 'Please enter Authorize1 ';
            $valid = false;
        }

        if (empty($Authorize2)) {
            $Authorize2Error = 'Please enter  Authorize2';
            $valid = false;
        }
        ...
        if (!empty($_FILES['image']['name']) && !$uploader->valid($_FILES['image']))
    {
        ...
        if ($valid) {
            ...
            $sql = "INSERT INTO tbl_consignee (Consignee,Consigneefull,ContactPerson,Designation,TelNo,Fax,Email,Address,City,Assignatory,AssigTitle,AssignatoryAddress,AssigStatus,TIN,Authorize1,Authorize2,image,Encodedby,EncodedDate) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $q = $pdo->prepare($sql);
            $q->execute(array($Consignee,$Consigneefull,$ContactPerson,$Designation,$TelNo,$Fax,$Email,$Address,$City,$Assignatory,$AssigTitle,$AssignatoryAddress,$AssigStatus,$TIN,$Authorize1,$Authorize2,$image,$Encodedby,$EncodedDate));
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="create_consignee.php" method="post" enctype="multipart/form-data">
...
<div class="control-group <?php echo !empty($Authorize1Error)?'error':'';?>">
  <label class="control-label">Authorize1</label>
  <div class="controls">
...
<div class="control-group <?php echo !empty($Authorize2Error)?'error':'';?>">
  <label class="control-label">Authorize2</label>
  <div class="controls">
